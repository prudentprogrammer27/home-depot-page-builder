import { useState } from "react";

const ConsumerCard =({currentProduct})=> {
    const [priceDollars, setPriceDollars] = useState(179);
    const [priceCents, setPriceCents] = useState('00');
    let cardDiscountPrice = (priceDollars - 25) + "." + priceCents;

    return (
        <div className="consumer-card-wrapper sb-parent">
            <img className="consumer-card-png" src="https://assets.thdstatic.com/images/v1/payment-credit-card-thd.png"></img>
            <div className="consumer-card info">
                <div>Pay <b>${cardDiscountPrice}</b> after <b>$25 OFF</b> your total qualifying purchase upon opening a new card.
                    <img className="consumer-card-png info" src="https://assets.thdstatic.com/images/v1/info.svg" title="Enjoy special savings with your new account"></img>
                </div>
                <a href="https://citiretailservices.citibankonline.com/apply/home-depot-credit-card?app=UNSOL&siteId=PLCN_HOMEDEPOT&sc=30005&cmp=A~P~U~O~3~9~ZZZ0~AI~HD~ZZ_1DT">Apply for a Home Depot Consumer Card</a>
            </div>
        </div>
    )
}

export default ConsumerCard;